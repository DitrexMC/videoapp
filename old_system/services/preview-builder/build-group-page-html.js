...
function buildGroupPageHtml({ group, files, fileCount, totalSize, categories, pageUrl, isOwner, ownerLabel, hideUploader }) {
    ...
    const fileRow = f => {
        ...
        const thumbHTML = showThumb
            ...
            : `<span class="gfi-icon">${getMimeIcon(f.mime_type)}</span>`;

        return `
        <a class="gfi" href="/f/${escapeHtml(f.id)}">
          <div class="gfi-thumb">
            ${thumbHTML}
          </div>
          <span class="gfi-ext">${escapeHtml(ext.toUpperCase())}</span>
          <div class="gfi-name">${escapeHtml(f.original_name)}</div>
          <div class="gfi-meta">${fmtBytes(f.size_bytes)}</div>
        </a>`;
    };
...
<div class="viewer">
  ...
  <div class="info-panel">
    <div class="info-inner">
      ...
      <div class="info-top">
        <div>
          <div class="file-name" id="group-name-display">${escapeHtml(groupName)}</div>
          <div class="file-mime">${fileCount}ファイル · ${categories.map(c => catLabelMap[c] || c).join(' · ')}</div>
        </div>
        <div class="actions">
          <a class="btn btn-primary" href="/g/${escapeHtml(group.id)}/download">
      ...
      ${isOwner ? `
      <div class="owner-section-title" style="margin-top:1rem">オーナー操作</div>
      <div class="owner-row">
        <label class="owner-label">グループ名変更</label>
        <div class="owner-rename-row">
          <input class="owner-input" id="group-rename-input" type="text" maxlength="255" value="${escapeHtml(groupName)}">
          <button type="button" class="btn btn-sm" id="group-rename-btn">保存</button>
        </div>
      </div>
      <div class="owner-row">
        <label class="owner-label">公開設定</label>
        <button type="button" class="btn btn-sm" id="group-privacy-btn">${group.is_private ? '非公開 → 公開にする' : '公開 → 非公開 にする'}</button>
      </div>` : ''}
...
(function(){
  ...
  if (renameBtn) {
    renameBtn.addEventListener('click', function() {
      ...
      if (!name) return;
      fetch('/api/user/groups/${escapeHtml(group.id)}', {
        method: 'PATCH',
  ...
  var privacyBtn = document.getElementById('group-privacy-btn');
  if (privacyBtn) {
    privacyBtn.addEventListener('click', function() {
      fetch('/api/user/groups/${escapeHtml(group.id)}', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ isPrivate: ${!!group.is_private} ? false : true }),
      }).then(function(r) { if (r.ok) location.reload(); });
    });
  }
