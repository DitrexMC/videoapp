    ...
    <tr style="white-space:nowrap">
      ...
      <td>
        <div style="display:flex;flex-direction:column;gap:4px;min-width:120px">
          <div style="font-size:.78rem;color:var(--text-sub)">${fmtBytes(u.used_bytes || 0)} / ${fmtBytes(u.quota_bytes)}</div>
          <div class="quota-bar-wrap"><div class="quota-bar" style="width:${usedPct.toFixed(1)}%"></div></div>
          <div style="font-size:.72rem;color:var(--text-sub)">${u.file_count || 0}件のファイル</div>
        </div>
      </td>
...
async function loadAdminFiles(search = '') {
    ...
    const [files, groups] = await Promise.all([
        fetch('/api/admin/files' + q).then(r => r.json()),
        fetch('/api/admin/groups' + q).then(r => r.json()),
    ]);
    ...
    <tr style="background:var(--bg3)">
      <td style="width:1%">
        <label class="custom-cb" style="padding:0;margin:0">
          <input type="checkbox" class="admin-file-check" data-id="g:${esc(item.id)}">
          <span class="custom-cb-box"></span>
        </label>
      </td>
      <td><a href="/g/${esc(item.id)}" style="color:var(--p);text-decoration:none;font-weight:700">
        グループ${item.label ? ' — ' + esc(item.label) : ''} (${item.file_count}件)</a></td>
      <td>—</td>
      <td>${fmtBytes(item.total_size)}</td>
      <td>${fmtDate(item.created_at)}</td>
      <td>${fmtDate(item.expires_at)}</td>
      <td><button class="btn btn-danger btn-sm admin-del-grp-btn" data-id="${esc(item.id)}">削除</button></td>
    </tr>`;
        }
        return `
    <tr>
      <td style="width:1%">
        <label class="custom-cb" style="padding:0;margin:0">
          <input type="checkbox" class="admin-file-check" data-id="f:${esc(item.id)}">
          <span class="custom-cb-box"></span>
        </label>
      </td>
      <td><a href="/f/${esc(item.id)}" style="color:var(--accent3);text-decoration:none">${esc(item.original_name)}</a></td>
      <td>${esc(item.user_label || '')}</td>
      <td>${fmtBytes(item.size_bytes)}</td>
      <td>${fmtDate(item.created_at)}</td>
      <td>${fmtDate(item.expires_at)}</td>
      <td><button class="btn btn-danger btn-sm admin-del-btn" data-id="${esc(item.id)}">削除</button></td>
    </tr>`;
    }).join('');
    ...
    document.querySelectorAll('.admin-del-grp-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
