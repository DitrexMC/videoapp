...
(function () {
    ...
    window.buildFileRow = function (item, opts) {
        ...
        if (showDl && item.status === 'ready') {
            ...
            actions.push('<button class="btn btn-ghost btn-sm fr-discord-btn" data-url="/files/' + frEsc(item.id) + '/download" style="padding:.22rem .55rem;font-size:.74rem;color:#7289da" title="Discord向け直リンクをコピー">' + DISCORD_ICON + '<span class="fr-action-label">Discord</span></button>');
        }
